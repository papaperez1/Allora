...
def calculate_volatility(df):
    """Calculate the price volatility as the standard deviation of returns."""
...
def format_data(files, data_provider):
    ...
    # Calculate volatility and save data
    price_df = calculate_volatility(price_df)
    price_df.sort_index().to_csv(training_price_data_path)

def load_frame(frame, timeframe):
    print(f"Loading data...")
    df = frame.loc[:, ['open', 'high', 'low', 'close', 'volatility']].dropna()
    df[['open', 'high', 'low', 'close', 'volatility']] = df[['open', 'high', 'low', 'close', 'volatility']].apply(pd.to_numeric)
